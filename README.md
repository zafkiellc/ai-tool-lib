...
## 来源与归集规则（重要）
- **本库是唯一真源**：历史上各端（WorkBuddy / Codex / DSH / Hermes）曾把成品工具散存在各自本机或私有 skills 目录。2026-08-27 已把 `工作/项目/资产/WorkBuddy私有Skills/`（WorkBuddy 端 2026-08-23 备份的 6 个私有 skill 副本）合并进本库并删除副本——那 6 个工具与本库完全一致（仅 permalink 细节差异），无独有经验丢失。
- **各端做的成品工具都要归集到这里**：任何端 AI 新做/改造的、可复用的成品工具（给具体平台用的脚本/skill/应用），**必须按「入库流程」放进本库对应 `<工具名>/` 目录**，不要只留在本机 `~/.workbuddy/skills/`、`~/.codex/` 或 DSH 本地。放进本库后随 verysync 各端一致、随 git 有版本，任何端都能继续优化。
- **私人向小工具**（如微信答题、三角洲改枪码等）保持 `私人/开发`、`私人/游戏` 原位，不进本库；本库只收纳「跨端共用、可复用」的工具。私人类在各自目录加 `[[AI工具库]]` 双链索引即可被各端发现。
- 第 7 个 `tencent-docs` skill 未进 vault（备份清单标注"如已随 WorkBuddy 分发则无需发布"），不丢。
- **迁移提示词已就绪**：`各端工具迁移提示词.md` 供各端 AI 把自己本地成品工具迁入本库并回报结构化结果，由路由端(Hermes)统一确认。新端首次接入用 `新端接入提示词.md`。回报统一写进 `迁移回报/<端名>__<AI名>.md`，看板见 `迁移状态看板.md`。

## 关联（库外，保持原位）
本库已收纳 9 个工具（6 自研 + 3 工作类应用，见目录）。以下成品应用由对应端做在 vault 其他位置，保持原位并通过双链纳入统一发现：
- **私人向**（保持原位，不进本库）
  - [[微信自动答题小工具]]（私人/开发）
  - [[三角洲改枪码数据管理工具]]（私人/游戏）
  - [[微信自动答题小工具]]（私人/开发）
